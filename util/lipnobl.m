...
nu_targets = 2;
%decis_thres = 28; %%% parameter for Cosyne
decis_thres = 58;
decis_flag = 0;
...
%max_rate_in2 = 30/1.5; %%% parameter for Cosyne
max_rate_in2 = 20;
b_in2 = 0;
K_in2 = 4;
...
% MT weight 
%g_w_oi = 14; %%% parameter for Cosyne
g_w_oi = 65;

% Visual evoked weights
%g_w_oi2= 4; %%% parameter for Cosyne
g_w_oi2= 40;
...
g_w_oo = 21;
amp_i = 0.0; 
%dc_w_oo = -0.078; %%% parameter for Cosyne
dc_w_oo = -0.17;
...
%    %%% proba_in: proba of firing of input neurons in response to motion
     ...
     proba_in = ((max_rate_in-b_in*0)*exp(K_in*(cos((pos_in'-pos)/180*2*pi)-1))+b_in*0)*delta_t; 
...
%                               +1/time_const_out*((w_oo-dc_w_oo)*spikes_out{1}(:,k));    
      ...
      for n=1:nu_window
         proba_count{m,n}(:,count)= ...
             sum(proba_out{m}(:,wind_size*(n-1)+1:wind_size*n)')'/wind_size;         
         spikes_count{m,n}(:,count)= ...
             sum(spikes_out{m}(:,wind_size*(n-1)+1:wind_size*n)')';
         spikes_count_in{m,n}(:,count)= ...
             sum(spikes_in{m}(:,1:wind_size*n)')';
      end
      end
